...
if (process.env.DATABASE_URL) {
  ...
  pool.query(`CREATE TABLE IF NOT EXISTS quotes (
    id SERIAL PRIMARY KEY, created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW(),
    rego TEXT DEFAULT '', vehicle JSONB DEFAULT '{}', damage JSONB,
    line_items JSONB DEFAULT '[]', total NUMERIC(10,2) DEFAULT 0
  )`).then(() => console.log("DB ready")).catch(err => console.error("DB init:", err.message));
}

app.get("/api/quotes", async (_req, res) => {
  if (!pool) return res.json([]);
  try { const { rows } = await pool.query("SELECT id,created_at,updated_at,rego,vehicle,damage,line_items,total FROM quotes ORDER BY updated_at DESC LIMIT 200"); res.json(rows); }
...
app.post("/api/quotes", async (req, res) => {
  ...
  try { const { rows } = await pool.query("INSERT INTO quotes (rego,vehicle,damage,line_items,total) VALUES ($1,$2,$3,$4,$5) RETURNING id", [rego||"", vehicle||{}, damage||null, JSON.stringify(lineItems||[]), total||0]); res.json({ id: rows[0].id }); }
  catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.put("/api/quotes/:id", async (req, res) => {
  if (!pool) return res.status(503).json({ error: "No database configured." });
  const { rego, vehicle, damage, lineItems, total } = req.body;
  try { await pool.query("UPDATE quotes SET rego=$1,vehicle=$2,damage=$3,line_items=$4,total=$5,updated_at=NOW() WHERE id=$6", [rego||"", vehicle||{}, damage||null, JSON.stringify(lineItems||[]), total||0, req.params.id]); res.json({ ok: true }); }
  catch (err) { res.status(500).json({ error: err.message }); }
});

app.delete("/api/quotes/:id", async (req, res) => {
  if (!pool) return res.status(503).json({ error: "No database configured." });
  try { await pool.query("DELETE FROM quotes WHERE id=$1", [req.params.id]); res.json({ ok: true }); }
  catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.post("/api/analyze", async (req, res) => {
  ...
  try {
    ...
    const imageBlocks = photos.slice(0,6).map(p => ({ type:"image", source:{ type:"base64", media_type:p.mime||"image/jpeg", data:p.base64 } }));
...
{"summary":"one-sentence overall assessment","panels":[{"panel":"name","severity":"minor|moderate|heavy|severe","repair_method":"PDR|Fill & Paint|Replace|Blend","labour_hours":number,"notes":"short note"}],"flags":["concerns"],"paint_complexity":"solid|metallic|pearl|tri-coat"}
Use standard collision-repair panel names.`;
    const r = await fetch("https://api.anthropic.com/v1/messages", { method:"POST", headers:{ "Content-Type":"application/json","x-api-key":API_KEY,"anthropic-version":"2023-06-01" }, body:JSON.stringify({ model:MODEL, max_tokens:1000, messages:[{ role:"user", content:[...imageBlocks,{ type:"text", text:prompt }] }] }) });
    if (!r.ok) return res.status(502).json({ error:"Claude API error: "+(await r.text()) });
    const data = await r.json();
    const text = data.content.filter(b=>b.type==="text").map(b=>b.text).join("");
    let parsed;
    try { parsed = JSON.parse(text.replace(/```json|```/g,"").trim()); } catch { return res.status(500).json({ error:"Could not parse AI response.", raw:text }); }
    res.json(parsed);
...
app.listen(PORT, () => console.log(`CrashQuote AI running on port ${PORT}`));// ─────────────────────────────────────────────────────────────
// CrashQuote AI — Backend Server
...
If you cannot clearly see damage, give your best professional estimate based on what's visible. Use standard collision-repair panel names.`;
    const anthropicResp = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({ model: MODEL, max_tokens: 1000, messages: [{ role: "user", content: [...imageBlocks, { type: "text", text: prompt }] }] }),
    });
    if (!anthropicResp.ok) {
